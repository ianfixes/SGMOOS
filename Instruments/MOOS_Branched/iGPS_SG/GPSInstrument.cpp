...
bool CGPSInstrument::OnStartUp()
{
    ...
    else
    {
        ...
        AddMOOSVariable("LAT","","GPS_LAT",dfGPSPeriod);
        AddMOOSVariable("LON","","GPS_LON",dfGPSPeriod);
    }
...
bool CGPSInstrument::GetData()
{
    if(!IsSimulateMode())
    {
        ...
        if (data.bGood)
        {
            SetMOOSVar("LAT", data.dfLat_deg,  dfTimeNow);
            SetMOOSVar("LON", data.dfLong_deg, dfTimeNow);
