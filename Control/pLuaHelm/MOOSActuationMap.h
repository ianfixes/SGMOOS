...
/**
This is the most basic of abstract base classes meant to simply define 
an interface for vehicle-specific child classes to utilize.
...
typedef struct
{
    bool (*withString)(string, string);
    bool (*withDouble)(string, double);
} MOOS_Notifier;

typedef map<string, CMOOSMsg> MSG_MAP;
...
public:
    CMOOSActuationMap();
    virtual ~CMOOSActuationMap();
    virtual bool Engage(const CPathAction& DesiredAction, MSG_MAP &msgs, MOOS_Notifier notify)=0;
    virtual STRING_LIST Registrations() const;
